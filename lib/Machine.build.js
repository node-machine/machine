...
module.exports = function Machine_build (machineDefinition) {// eslint-disable-line camelcase
  ...
  if (_.keys(machineDefinition.inputs).length > 0) {
    ...
    _.each(_.keys(machineDefinition.inputs), function(inputKey) {
      ...
      // If `allowNull` is set, the input must be a literal type, and must not be required.
      if (input.allowNull) {
        if (input.example === '*' || input.example === '===') {
          throw flaverr('MACHINE_INPUT_INVALID', new Error(
            'Failed to build `' + (machineDefinition.identity || 'anonymous') + '` from the specified definition.\n'+
            'The `'+inputKey+'` input has `allowNull: true`, which is unnecessary based on the input\'s declared type.\n'+
            GENERIC_HELP_SUFFIX
          ));
        }
        if (input.required === true) {
          throw flaverr('MACHINE_INPUT_INVALID', new Error(
            'Failed to build `' + (machineDefinition.identity || 'anonymous') + '` from the specified definition.\n'+
            'The `'+inputKey+'` input has `allowNull: true`, which cannot be used in conjunction with `required: true`.\n'+
            GENERIC_HELP_SUFFIX
          ));
        }
      }
    ...
    if(_errors.length > 0) {
      throw flaverr({ code: 'MACHINE_INPUT_INVALID' }, new Error(
        'Failed to build `' + machineDefinition.identity + '` from the specified definition.\n'+
        'The following properties in `inputs` are not valid input definitions: \n'+
        '• ' + _errors.join(',\n• ')+'\n'+
        GENERIC_HELP_SUFFIX
      ));
    }//--•
