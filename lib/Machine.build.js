  ...
  if (!_.isUndefined(defToResolve.like)) {
    ...
    if (_.isUndefined(referencedInput.example)) {
      throw (function (){
        ...
        'Failed to instantiate machine ("%s") from the specified machine definition.\n'+
        'An input\'s `like` should refer to a machine input with an explicit example (i.e. it can\'t use `like` or `itemOf` too!)  The referenced input (named `%s`) does not have an explicit example.',
        machineDefinition.friendlyName,
        defToResolve.like));
        err.machine = machineDefinition.identity;
        err.code = 'UNRECOGNIZED_INPUT';
  ...
  // If `itemOf` was provided, return the pattern of the array example from the specified input.
    ...
    if (_.isUndefined(referencedInput.example)) {
      throw (function (){
        var err = new Error(util.format(
        'Failed to instantiate machine ("%s") from the specified machine definition.\n'+
        'An input\'s `itemOf` should refer to a machine input with an explicit example (i.e. it can\'t use `like` or `itemOf` too!)  The referenced input (named `%s`) does not have an explicit example.',
        machineDefinition.friendlyName,
        defToResolve.itemOf));
        err.machine = machineDefinition.identity;
        err.code = 'UNRECOGNIZED_INPUT';
