...
module.exports = function Machine_build (machineDefinition) {
  ...
  _.each(machineDefinition.exits, function(exitDef, exitId) {
    ...
    // But if both were specified, throw an error.
    if (!_.isUndefined(exitDef.outputExample) && !_.isUndefined(exitDef.example)) {
      var ambiguousOutputExampleErr = new Error(
        'Failed to instantiate machine "' + machineDefinition.friendlyName + '" from the specified machine definition.\n'+
        'The machine\'s `'+exitId+'` exit specifies both an `outputExample` and an `example` -- but since `example` is just '+
        'an alias for `outputExample`, it is unclear which should be used.'
      );
      ambiguousOutputExampleErr.code = 'MACHINE_EXIT_INVALID';
      throw ambiguousOutputExampleErr;
    }
