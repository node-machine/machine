      ...
      if (input.allowNull) {
        if (input.example === '*' || input.example === '===') {
          throw flaverr('MACHINE_INPUT_INVALID', new Error(
            ...
            'The `'+inputKey+'` input has `allowNull: true`, which is not valid for the specified input type.\n'+
            GENERIC_HELP_SUFFIX
          ));
        }
        if (input.required === true) {
          throw flaverr('MACHINE_INPUT_INVALID', new Error(
            'Failed to build `' + (machineDefinition.identity || 'anonymous') + '` from the specified definition.\n'+
            'The `'+inputKey+'` input has `allowNull: true`, which is not valid in conjunction with `required: true`.\n'+
            GENERIC_HELP_SUFFIX
          ));
        }
      }
