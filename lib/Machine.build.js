...
module.exports = function Machineºbuild (machineDefinition) {
  ...
  _callableMachineWrapper.inspect = function (){
    return util.format(
      '-----------------------------------------\n'+
      ' [Machine: %s]\n\n'+
      ' Inputs:\n'+
      (_.reduce(_callableMachineWrapper.inputs, function (memo, inputDef, inputName){

        memo += util.format('  • %s\n',inputName);

        if (inputDef.example) {
          memo += util.format('    e.g. %s\n', typeof inputDef.example === 'string' ? '"'+inputDef.example+'"' : typeof inputDef.example === 'object' ? '\n'+util.inspect(inputDef.example, false, null) : inputDef.example);
        }
        else if (inputDef.type) {
          memo += util.format('    (type: %s)\n', inputDef.type);
        }
        return memo;
      }, '')||' (n/a)')+'\n'+
      '-----------------------------------------\n',
      machineDefinition.id
    );
  };
