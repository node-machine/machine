...
module.exports = function Machineºbuild (machineDefinition) {
  ...
  if (_.keys(machineDefinition.inputs).length) {
    ...
    _.each(_.keys(machineDefinition.inputs), function(inputKey) {
      ...
      if(input.typeclass) {
        if(['array', 'dictionary', 'primative'].indexOf(input.typeclass) < 0) {
          _errors.push(inputKey);
        }
      }

      if(_.isUndefined(input.example) && _.isUndefined(input.typeclass) && _.isUndefined(input.validate) && _.isUndefined(input.getExample))  {
        _errors.push(inputKey);
      }
    });
    ...
    e.message = e.message + '\n' + util.format(
      'Failed to instantiate machine from the specified machine definition.\n'+
      'The following `input` properties are missing an `example` property: \n%s',
    '• ' + _errors.join(',\n• '));
  ...
  _callableMachineWrapper.setEnvironment = function (){
    var machineInstance = new Machine(machineDefinition);
    return machineInstance.environment.apply(machineInstance, Array.prototype.slice.call(arguments));
  };
