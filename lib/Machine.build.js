      ...
      if (input.type) {
        ...
        // Otherwise make sure that the given example is compatible with the given type.
          ...
          try {
            ...
            throw flaverr('MACHINE_INPUT_INVALID', new Error('Failed to instantiate machine "' + machineDefinition.friendlyName + '" from the specified machine definition.\nThe machine\'s `'+inputKey+'` input has `type` "' + input.type + '", but the `example` is not valid for that type.  If both `type` and `example` are provided, they must be compatible!'));
          }
        }
