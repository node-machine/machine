...
function Machine (machineDefinition) {
  ...
  var self = this;
  this._onInvoke = function defaultOnInvoke () {
    /**
     * Default `onInvoke` handler
     * @logs {String,String,...}
     */
    (Machine.onInvoke||function _defaultPostInvocationHandler(msElapsed){
      var elaborationPhrase = (function _determineElaborationPhrase(){
        var phrase = (self.description||self.friendlyName) ? '    ('+(self.description||self.friendlyName)+')' : '';
        if (phrase.length > 80){
      ...
      var logMsg = util.format('machine: `%s` -- %dms', self.identity, msElapsed);
      logMsg += _.reduce(_.range(50 - logMsg.length), function (memo){return memo+' ';}, '');
      self.log(logMsg, elaborationPhrase);
...
Machine.prototype.setExits = function (configuredExits) {
  ...
  // Support traditional callback usage
  if (self.defaultExit && self.defaultExit !== 'success') {
  ...
  // Now make all other exits in exit schema forward to the catchall exit
