...
Machine.prototype.setExits = function (configuredExits) {
  ...
  // Fill in any unhandled exits (ignoring the default exit) and have them redirect to the `error` (i.e. catchall) exit
  _.each(_.keys(self.exits), function (exitName) {
    ...
    if (!configuredExits[exitName]) {
      configuredExits[exitName] = function forwardToErrorExit (_resultPassedInByMachineFn){
        ...
        // ...so this should work even if no error callback was explicitly confiured)
