...
Machine.prototype.exec = function (configuredExits) {
  ...
  (function _tryCacheLookup (giveUpAndRunMachineCb) {
    ...
    // Run hash function to calculate appropriate `id` criterion
    hash = _hashFn(self._configuredInputs);
    ...
    var interceptedExits = _.reduce(self._configuredExits, function (m,fn,exitName){
      ...
      // Don't mess with this exit if we're not caching this machine's results
      // or if this exit is not the cacheable exit
      if (!_cache || exitName !== _cache.exit) {
        m[exitName] = fn;
        return m;
      }
