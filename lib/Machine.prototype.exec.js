...
module.exports = function Machine_prototype_exec (done) {
  ...
  // console.log('CLEAN ERROR STACK when .exec()ing machine `' + self.identity +'`\n', cleanError.stack);
  // console.log('');console.log('');console.log('');
  cleanError.stack = (function makeNiceCleanErrorStack() {
    var stackLines = cleanError.stack.split('\n');
    // Lop off the first line of the stack trace, which we'll replace
    // with a better message below, and the second line, since it's
    // just a reference to this file (.exec)
    stackLines = stackLines.slice(2);
    // Lop off the second, third and fourth lines of those remaining,
    // since they will also be referring to `.exec()`
    stackLines.splice(1,3);
    // Join the lines back together
    return stackLines.join('\n');
  })();

  // Set up a local variable (`appendedProvidedStackFromEnv`) to serve as
  // a hacky (and hopefully temporary) flag to indicate whether a custom
  // stack trace was concatenated with the auto-generated clean error stack
  // trace, and passed in to this machine's `env` as `env.stack`.
  var appendedProvidedStackFromEnv;
  // TODO: remove this (^^) flag-setting hack... erm, I think.. maybe  (but see other related TODOs for context)
  // ***NOTE: I think we might NEED a flag-- but not exactly this flag.  More like "didPassThroughEnvStack"... or something like that.***

  // self._configuredEnvironment.stack.stack = (cleanError.stack.split('\n').slice(1).join('\n')+'\n'+self._configuredEnvironment.stack.stack);
  // cleanError.stack = self._configuredEnvironment.stack.stack;
    ...
    if (errors.length > 0) {
      ...
      if(self._runTimeTypeCheck) {
        ...
        var err_machineRuntimeValidation = (function _buildMachineRuntimeValidationErr() {
          ...
          // Start with the auto-generated `cleanError`.
          var err_machineRuntimeValidation = cleanError;

          // If a custom `stack` was provided via `.setEnv()`, then we'll assume it has all
          // of the relevant context for debugging, and we won't concatenate our automatically
          // generated stack trace from (`cleanError`).
          if (!appendedProvidedStackFromEnv) {
            err_machineRuntimeValidation.stack = getCombinedErrorStack(cleanError, self._configuredEnvironment.stack);
          }
          // ==============================================================================
          // TODO: ^^ ACTUALLY I think that, instead of checking if a stack was PROVIDED via setEnv,
          // maybe we need to check whether a stack trace was PASSED THROUGH to the machine `fn`.
          // Just a hunch though-- need to follow up.
          // -m sep 13, 2016
          // ==============================================================================
  ...
  (function _doCacheLookupMaybe (cb) {
    ...
    _.each(_.keys(self.exits), function (exitCodeName) {
      ...
      self._configuredExits[exitCodeName] = function __triggeredMiscExit(_resultPassedInByMachineFn){
        ...
        // generated stack trace from (`cleanError`).
        if (!appendedProvidedStackFromEnv) {
          err_forwarding.stack = getCombinedErrorStack(cleanError, self._configuredEnvironment.stack);
        }
        // ==============================================================================
        // TODO: ^^ ACTUALLY I think that, instead of checking if a stack was PROVIDED via setEnv,
        // maybe we need to check whether a stack trace was PASSED THROUGH to the machine `fn`.
    ...
    try {
      ...
      //  └─     ┴ └─┘  ┴  ┴ ┴└─┘└─┘  ┴┘└┘  └─┘└─┘┴─┘└─┘└┴┘    ─┘
      //
      // Build `envToPassIn`, a new dictionary which we'll eventually provide as the third
      // argument (`env`) when we invoke this machine's `fn` below.
      var envToPassIn = _.clone(self._configuredEnvironment);


      // If an error stack string was provided via `setEnv()`, then concatenate
      // that custom stack with our automatically-generated "clean" error stack.
      // We'll pass this COMBINED stack string into our machine's `fn` as `env.stack`.
      if (self._configuredEnvironment.stack) {

        var combinedStack = cleanError.stack + '\n=================\n'+self._configuredEnvironment.stack;
        envToPassIn.stack = combinedStack;
        // console.log('ADDING STACK FROM ENV (running exec for machine `' + self.identity + '`)\n', self._configuredEnvironment.stack);
        appendedProvidedStackFromEnv = true;
        // TODO: remove this (^^) flag-setting hack... erm, I think  (but see other related TODOs for context)

      }// ‡
      // Otherwise, if the userland code (i.e. that triggered this `exec()`) disabled the
      // automatically generated stack trace by setting `stack: false`, then don't pass
      // in anything as `env.stack`.
      //
      // > To do this, run:  `.setEnv({ stack: false })`
      // > (useful for secondary tooling like machine-as-action, etc)
      else if (self._configuredEnvironment.stack === false) {

        delete envToPassIn.stack;

      }//‡
      // Last but not least, we have the general case: where the userland code made this
      // call to `.exec()` did not explicitly DISABLE the automatic creation of `env.stack`,
      // but where they didn't PROVIDE their own stack via `.setEnv()` either.
      else {

        // In this case, we'll simply use our automatically-generated "clean" error stack.
        // We'll pass in this auto-generated stack as `env.stack` when we invoke our machine's `fn` below.
        envToPassIn.stack = cleanError.stack;

      }//</else :: no custom stack trace was provided via setEnv(), and the auto-trace was NOT disabled via setEnv({stack:false})>


      // Now, do a quick assertion as a sanity check.
      // > This is a failsafe to make sure the code above ALWAYS does a shallow clone.
      if (envToPassIn === self._configuredEnvironment.stack){
        throw new Error('Consistency violation: `envToPassIn` should NEVER be a reference to the original `self._configuredEnvironment` (should have done a shallow clone!)');
      }
      ...
      try {
        self.fn.apply(envToPassIn, [self._configuredInputs, implementorSwitchback, envToPassIn]);
      } catch (e) {
