...
module.exports = function Machine_prototype_exec (configuredExits) {
  ...
  try {
    coercedInputValues = validateConfiguredInputValues(self);
  } catch (e) {
    // console.log('got error:',e);
    // If run time type checking is enabled, trigger the error exit
    // if any inputs are valid.
    ...
    if(self._runTimeTypeCheck) {
      ...
      return switchback(self._configuredExits, undefined, undefined, true)(e);
    }
  }

  // If the `_inputCoercion` flag is enabled, configure the machine with the
  // newly coerced input values.
  // console.log('_inputCoercion:',self._inputCoercion);
