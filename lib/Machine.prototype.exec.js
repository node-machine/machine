    ...
    if (errors.length > 0) {
      ...
      if(self._runTimeTypeCheck) {
        ...
        return switchback(self._configuredExits, undefined, undefined, true)((function (){
          ...
          var err = new Error(util.format('`%s` machine encountered %d error(s) while validating runtime input values: ', self.identity, errors.length, _.pluck(errors, 'message').join('\n') ));
