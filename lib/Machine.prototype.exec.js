...
module.exports = function Machine_prototype_exec (done) {
  ...
  // > (Note that, technically, `done` is _always_ required.  See below for why.)
  if (done !== undefined) {
    ...
    if (_.isArray(done) || (!_.isFunction(done) && !_.isObject(done))) {
      throw new Error('Invalid usage: `.exec()` must be called with either:\n'+
    ...
    // Finally, if a dictionary of individual callbacks was provided, handle them accordingly.
      ...
      var allDeclaredExitCodeNames = _.keys(this.exits);
      var unrecognizedExitCodeNames = _.difference(_.keys(done), allDeclaredExitCodeNames);
      if (unrecognizedExitCodeNames.length > 0) {
        throw new Error('Invalid usage: One or more callbacks were configured for exits that are not recognized by this machine: `' + unrecognizedExitCodeNames.join(', ') + '`.');
      }
      // Tack these individual callbacks onto the dictionary of configured exit callbacks.
      _.extend(this._configuredExits, done);
