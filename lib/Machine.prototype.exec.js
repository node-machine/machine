...
module.exports = function Machineºprototypeºexec (configuredExits) {
  ...
  if(this._runTimeTypeCheck) {
    var typeDef = setTypes(this.inputs);

    try {
      T.rttc(typeDef, this._configuredInputs, { coerce: this._inputCoercion });
    } catch (err) {
      switchback(this._configuredExits)(err);
      return;
    }
  }
  ...
  function _runMachine (err) {
    ...
    var interceptedExits = _.reduce(self._configuredExits, function (m,fn,exitName){
      ...
      if (!_cache || !hash || exitName !== _cache.exit) {
        m[exitName] = function ensureIsAsynchronous(value) {
    ...
    // Run the machine
    try {
      self.fn.apply(self._configuredEnvironment, [self._configuredInputs, switchback(interceptedExits), self._configuredEnvironment]);
      return;
    }
    catch(e) {
      return switchback(interceptedExits)(e);
    }
  }
...
/**
 * Prune undefined values from the specified array or object.
 *
...
function pruneUndefined(thing) {
  return _.remove(thing, function (item) {
