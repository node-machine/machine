...
module.exports = function Machineºprototypeºexec (configuredExits) {
  ...
  if(this._runTimeTypeCheck) {
    ...
    try {
      T.rttc(typeDef, this._configuredInputs);
    } catch (err) {
  ...
  (function _tryCacheLookup (giveUpAndRunMachineCb) {
    ...
    var interceptedExits = _.reduce(self._configuredExits, function (m,fn,exitName){
      ...
      if (!_cache || !hash || exitName !== _cache.exit) {
        m[exitName] = function ensureIsAsynchronous( /* arguments */){
          // Save the original args, since "arguments" will be redefined in the setTimeout callback
          var args = arguments;
          setTimeout(function (){
            return fn.apply(self._configuredEnvironment, Array.prototype.slice.call(args));
          }, 0);
        };
        return m;
      }
