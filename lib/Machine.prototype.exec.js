...
var hashArgins = require('./private/hash-argins');
var interceptExitCallbacks = require('./private/intercept-exit-callbacks');
var validateConfiguredInputValues = require('./private/validate-configured-input-values');
var buildLamdaMachine = require('./private/build-lamda-machine');
var getCombinedErrorStack = require('./private/get-combined-error-stack');
var removeNonUserlandCallsFromStack = require('./private/remove-non-userland-calls-from-stack');
var getFilepathFromStackEntry = require('./private/get-filepath-from-stack-entry');
 ...
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 */
module.exports = function exec(done) {
  ...
  if (done !== undefined) {
    ...
    if (_.isArray(done) || (!_.isFunction(done) && !_.isObject(done))) {
      throw flaverr(new Error('Invalid usage: `.exec()` must be called with either:\n'+
  ...
  if (self._inputCoercion) {
    ...
    _.extend(this._configuredInputs, potentiallyCoercedArgins);
  }
