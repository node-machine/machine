...
module.exports = function Machine_prototype_exec (configuredExits) {
  ...
  // This local variable (`potentiallyCoercedInputValues`) is used below to hold a dictionary
  ...
  var potentiallyCoercedInputValues;
  ...
  if (!self._unsafeMode) {
    ...
    potentiallyCoercedInputValues = looseValidationReport.values;
    ...
    if (errors.length > 0) {
      ...
      if(self._runTimeTypeCheck) {
        ...
        return sb(err_machineRuntimeValidation);
  ...
  if (self._inputCoercion) {
    self.setInputs(potentiallyCoercedInputValues);
  }
  ...
  (function _cacheLookup (cb) {
    ...
    calculateHash(self, function (err, _calculatedHash){
      ...
      .exec(function (err, cached) {
        ...
        // Cache miss
        return cb();
      });//</_cache.model.find() :: finding records in cache model>
