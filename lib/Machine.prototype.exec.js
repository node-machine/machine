    ...
    if (errors.length > 0) {
      ...
      if(self._runTimeTypeCheck) {
        ...
        return switchback(self._configuredExits, undefined, undefined, true)((function (){
          ...
          var err = new Error(
            'Could not run `'+self.identity+'` due to '+errors.length+' validation error'+(errors.length>1?'s':'')+' '+
            'encountered when using RTTC to verify the data types of the provided runtime input values.  '+
            'Details:\n',
            errors.join('\n')
          );
