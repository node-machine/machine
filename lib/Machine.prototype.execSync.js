  ...
  _.each(this.exits, function (exitDef, exitName) {
    ...
    exitCallbacks[exitName] = function (_data){
      ...
      // (if not, build it as an error object using the exit definition)
        ...
        var exitDescription = self.exits[exitName] && self.exits[exitName].description;
        var exitHasOutput = self.exits[exitName] && !_.isUndefined(self.exits[exitName].example);
        var errMsg = util.format(
          '`%s` triggered its `%s` exit%s%s',
          self.identity,
          exitName,
          (exitDescription ? (':\n'+exitDescription) : ''),
