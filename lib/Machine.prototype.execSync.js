...
module.exports = function (){
  ...
  // as synchronous-- or provided a `getSync()` function to make
  // this decision at runtime (i.e. now).
  if (!this.sync) {

    // Try running `getSync()` if one was provided:
    if (_.isFunction(this.getSync)) {

      // Note that we do not clone input values before passing them
      // to `getSync()`.  This is an advanced feature, and the responsibility
      // for avoiding entanglement rests with the implementor.
      try {
        this.sync = this.getSync.apply(env, [this._configuredInputs, env]);

        if (!this.sync) {
          // If `getSync` returned falsy, this
          // then this is a usage error.
          throw (function (){
            var _err = new Error('Cannot use `.execSync()` with `'+identity+'` machine because its `getSync()` function returned falsy-- meaning it does not enable synchronous usage for the given inputs.');
            _err.code = 'E_USAGE';
            _err.status = 400;
            return _err;
          })();
        ...
        throw (function (){
          var _err = new Error(util.format('An error (E_EXIT_GETSYNC) was thrown in the machine\'s (%s) `getSync()` function:\n', this.identity, util.inspect(e)));
          _err.code = 'E_EXIT_GETSYNC';
          return _err;
        })();
      }
    }

    // If neither `getSync` nor `sync` was provided (or `sync` was false),
    // then this is a usage error.
    throw (function (){
