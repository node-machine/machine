 ...
 * @param  {Object} inputs
 * @param  {Object} configuredInputValues
 */

module.exports = function Machine_setTypes (inputs, configuredInputValues) {
  ...
  _.each(_.keys(inputs), function(inputName) {
    ...
    if(input.validate) {
      try {
        ...
        var valid = input.validate.call(validateEnv, inputs, env);
        if(!valid) {
        ...
        errors.push(inputName + ' does not contain valid input values.  Error: \n'+util.inspect(e, false, null));
        return;
      }
    ...
    // Run input's `getExample()` function, if one exists.
      ...
      try {
        ...
        input.example = input.getExample.call(getExampleEnv, configuredInputValues, env, configuredInputValues[inputName]);
    ...
    // Otherwise, check the input's explicit `typeclass`
