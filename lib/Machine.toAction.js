...
module.exports = function MachineºtoAction(machineDef) {
  var Machine = this;

  return function _wrappedAction(req, res, next) {
    var m = new Machine(machineDef);

    // Configure and run machine.
    return m.configure((function _buildInputs() {
      var inputValues = {};

      // Only add `sails` as an input if it exists.
      if (typeof sails !== 'undefined' || req._sails) {
        inputValues.sails = sails || req._sails;
      }
