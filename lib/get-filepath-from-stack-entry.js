...
module.exports = function getFilepathFromStackEntry (stackEntry) {
  ...
  try {
    ...
    // This is the regex from felixge's `stack-trace` package (https://www.npmjs.com/package/stack-trace),
    ...
    // If we found a matching file path in this stack entry, then return it.
    if (match && match[2]) {
      return match[2];
    }
