        ...
        (function maybeWait(cb){
          ...
          if (machine._isLogEnabled){
            ...
            try {
              ...
              console.log('machine: `%s` -- %dms', machine.identity, msElapsed, (machine.description||machine.friendlyName) ? '    ('+(machine.description||machine.friendlyName)+')' : '');
            }
            catch (e) {
              console.error('Error calculating/logging machine execution time\n:',e);
            }
          }
...
function getExample (exitDef, exitName, allConfiguredInputValues) {
  ...
  try {
    ...
    throw (function (){
      return new Error(util.format('`%s` exit: an error was thrown in the exit\'s `getExample()` function:\n', exitName, util.inspect(e)));
    })();
  }
