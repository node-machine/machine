...
function getExample (exitDef, exitName, allConfiguredInputValues) {
  ...
  if (!_.isUndefined(exitDef.like)) {
    return allConfiguredInputValues[exitDef.like];
  }

  // If `itemOf` was provided, look up the value of the referenced input and use it.
  if (!_.isUndefined(exitDef.itemOf)) {
    // If the runtime value is an empty array, fall back to using the more-generic
    // example we already figured out when building this machine instance.
    if (allConfiguredInputValues[exitDef.itemOf][0].length === 0) {
      return exitDef.example;
    }
    return allConfiguredInputValues[exitDef.itemOf][0];
  }
