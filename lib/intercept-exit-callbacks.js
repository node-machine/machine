...
function getExample (exitDef, exitName, allConfiguredInputValues) {
  ...
  if (!_.isUndefined(exitDef.like)) {
    ...
    //  statisfy BOTH schemas-- i.e. the intersection.  This should always work
    //  because, at this point, input validation has already run.)
    return rttc.intersection(exitDef.example, allConfiguredInputValues[exitDef.like], true, true);
  }
  ...
  if (!_.isUndefined(exitDef.itemOf)) {
    ...
    if (!allConfiguredInputValues[exitDef.itemOf] || allConfiguredInputValues[exitDef.itemOf][0].length < 1) {
    ...
    //  statisfy BOTH schemas-- i.e. the intersection.  This should always work
    //  because, at this point, input validation has already run.)
    return rttc.intersection(exitDef.example, allConfiguredInputValues[exitDef.itemOf][0], true, true);
  }
