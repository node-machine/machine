...
function getExample (exitDef, exitName, allConfiguredInputValues, machineDef) {
  ...
  if (!_.isUndefined(exitDef.like)) {
    ...
    return rttc.intersection(machineDef.inputs[exitDef.like].example, allConfiguredInputValues[exitDef.like], true, true);
  }
  ...
  if (!_.isUndefined(exitDef.itemOf)) {
    ...
    if (!allConfiguredInputValues[exitDef.itemOf] || allConfiguredInputValues[exitDef.itemOf][0].length < 1) {
      return machineDef.inputs[exitDef.itemOf].example[0];
    }
    ...
    //  Note that, input validation/coercion has already run- so this should never fail)
    return rttc.intersection(machineDef.inputs[exitDef.itemOf].example[0], allConfiguredInputValues[exitDef.itemOf][0], true, true);
  }
