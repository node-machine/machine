...
/**
 * `loadAndBuildCallableMachines()`
 *
 * Load modules in the specified directory and expose them as
 * a dictionary of wet machine instances.
 *
...
module.exports = function loadAndBuildCallableMachines (options) {
  ...
  if (_.isString(options)) {
    ...
    throw new Error('Usage error: `.loadAndBuildCallableMachines()` expects a dictionary of options, but instead got:'+util.inspect(options, {depth:null}));
  }

  // Validate `dir`
  if (_.isUndefined(options.dir)) {
    throw new Error('Usage error: `.loadAndBuildCallableMachines()` should be provided a `dir` option, but it was not provided.  Received options:'+util.inspect(options, {depth:null}));
  }
  if (!_.isString(options.dir)) {
    throw new Error('Usage error: `.loadAndBuildCallableMachines()` received a `dir` path which is not a string:'+util.inspect(options.dir, {depth:null}));
  }
  ...
  if (!_.isUndefined(options.pkg)) {
    if (!_.isObject(options.pkg) || _.isArray(options.pkg) || _.isFunction(options.pkg)) {
      throw new Error('Usage error: `.loadAndBuildCallableMachines()` received an invalid `pkg`.  If specified, `pkg` must be a dictionary, but instead got:'+util.inspect(options.pkg, {depth:null}));
    }
  }
  ...
  var Machine = this;
  if (!_.isFunction(Machine.build)) { throw new Error('Consistency violation: Context (`this`) is wrong in .loadAndBuildCallableMachines()!'); }
