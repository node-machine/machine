...
module.exports = function pack(options){
  ...
  Object.defineProperty(PackedModules, 'customize', {
    enumerable: false,
    configurable: false,
    writable: false,
    value: function customize(customUsageOpts){
      if (customUsageOpts.def) { throw new Error('Consistency violation: `def` is not a valid usage opt when calling `.customize()`!'); }
  ...
  Object.defineProperty(PackedModules, 'inspect', {
    enumerable: false,
    configurable: false,
    writable: true,
    value: function inspect(){
      ...
      return ''+
      '-----------------------------------------\n'+
      ' ['+(options.pkg.name || '[anonymous package]')+']\n'+
      (
        options.pkg.description ? (' '+options.pkg.description+'\n') : ''
      )+
      ' \n'+
      // FUTURE: include summary of all available methods here
      // ' Methods:'+
      // (
      //   _.reduce(options.pkg.machinepack.machines, function (memo, nmIdentity){
      //     memo += '\n '+('  - ')+' '+nmIdentity;

      //     if (!_.isUndefined(nmIdentity) {
      // todo: method name
      //       memo += '      +nmIdentity)';
      //     }
      //     return memo;
