 ...
 *
 *          @property {Dictionary?} customUsageOpts
 *              Only supported if `defs` is used.  Custom usage options to apply to every
 *              Callable that is auto-built by .pack().
 *
...
module.exports = function pack(options){
  ...
  //
  // FUTURE: Bring in the rest from parley too
  ...
  Object.defineProperty(newPack, 'customize', {
    ...
    value: function customize(customUsageOpts){
      ...
      // (this includes all the goodies like `.inspect()`)
      var customPack = pack({
        ...
        customUsageOpts: customUsageOpts,
  ...
  if (options.pkg !== undefined) {
    ...
    newPack = _.reduce(machines, function (memo, machineID) {
      ...
      try {
        ...
        // Build the machine.
        var machineInstance = helpBuildMachine({ def: definition });
  ...
  // ```
    ...
    _.each(options.defs, function(def, identity){
      ...
      var callable;
      if (options.customUsageOpts) {
        callable = helpBuildMachine(_.extend({
          def: _.extend({ identity: identity }, def),
        }, options.customUsageOpts));
      } else {
        callable = helpBuildMachine({
          def:_.extend({ identity: identity }, def)
        });
      }
