...
module.exports = function pack(optionsOrDir){
  ...
  Object.defineProperty(newPack, 'registerMethods', {
    enumerable: false,
    configurable: false,
    writable: true,
    value: function registerMethods(newMethodsByIdentity){
      var newMethodsByMethodName = _.reduce(newMethodsByIdentity, function(newMethodsByMethodName, def, identity){
        var methodName = getMethodName(identity);
        newMethodsByMethodName[methodName] = def;
        return newMethodsByMethodName;
      }, {});

      // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
      // FUTURE: Add "UNCONVENTIONAL_OR_RESERVED_METHOD_NAMES" check here, like
      // the one we do below.  (Would want to extrapolate that code into a private
      // utility.)
      // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

      _.extend(defs, newMethodsByMethodName);
    }//ƒ
  });
  ...
  var UNCONVENTIONAL_OR_RESERVED_METHOD_NAMES = [
    ...
    'customize', 'configure', 'registerMethods'
  ];
