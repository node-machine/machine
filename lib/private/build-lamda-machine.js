 ...
 * buildLamdaMachine()
 *
 * Build a submachine instance (aka "lamda machine").
 *
...
module.exports = function buildLamdaMachine(runtimeLamdaFn, nameOfLamdaInput, parentMachine, rootMachine){
  ...
  var lamdaMachineDef = {
    ...
    sync: _.isUndefined(parentLamdaInput.contract.sync) ? undefined : parentLamdaInput.contract.sync,
    ...
    inputs: _.reduce(parentLamdaInput.contract.provides||parentLamdaInput.contract.inputs, function buildEachInputDefForLamdaMachine(memo, def, name){
      ...
      // Because this is the machine instantiated for a lamda implementation,
      // we must deduce the appropriate example for inputs/exits using `like` or `itemOf`.
      if (_.isString(def.like)) {
        if (!rootMachine.inputs[def.like]) {
          throw new Error(util.format('Could not find input `%s` referenced by `like` in root machine (%s)', def.like, rootMachine.identity));
        }
        memo[name].example = rootMachine.inputs[def.like].example;
        delete memo[name].like;
      }
      else if (_.isString(def.itemOf)) {
        if (!rootMachine.inputs[def.itemOf]) {
          throw new Error(util.format('Could not find input `%s` referenced by `itemOf` in root machine (%s)', def.itemOf, rootMachine.identity));
        }
