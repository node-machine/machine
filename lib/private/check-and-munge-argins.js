...
module.exports = function checkAndMungeArgins(argins, metadata, nmDef){
  ...
  if (!getIsProduction()) {
    ...
    if (metadata !== undefined && (!_.isObject(metadata) || _.isArray(metadata) || _.isFunction(metadata))) {
      throw new Error('Consistency violation: If provided, `metadata` must be a dictionary.  But instead got: '+util.inspect(metadata, {depth: 5}));
    }
