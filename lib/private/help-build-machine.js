...
module.exports = function helpBuildMachine(opts, btOmen) {
  ...
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  // FUTURE: Consider changing this default back:
  // ```
  // var resultValidationTactic = opts.resultValidationTactic || 'forceCoerceAndClone';
  // ```
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  ...
  var wetMachine = function runFn(_argins, _explicitCbMaybe, _metadata){
    ...
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    if (!getIsProductionWithoutDebug()) {
      rtOmen = flaverr({ name: 'Omen' }, new Error('Runtime'), runFn);
    }//ﬁ
          ...
          case undefined:
                ...
                handlerCbs.error = function(rawOutput){
                  ...
                  if (undefined === rawOutput) {
                    ...
                    if (rawOutputParsedAsErr) {
                      ...
                      // Check the Error instance to see if it has a `code` or `name` that indicates any special meaning
                      // -- whether that be a `name` of `RuntimeValidationError`/ `TimeoutError`, or just a `code` that
                      // happens to overlap with the name of one of this machine's declared exits.  In either of these
                      // cases, strip that information off and preserve it instead.  This allows us to correctly treat
            ...
            if (nmDef.fn.constructor.name === 'AsyncFunction') {
              ...
              try {
                ...
                // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
                // ªIf a string was thrown from the implementation, then look to see if
                // it matches the name of an exit.  If so, then consider this equivalent
                ...
                // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
                if (_.isString(err) && implSideExitHandlerCbs[err]) {
                  return implSideExitHandlerCbs[err]();
                }
                else {
              ...
              promise.catch(function(err) {
                // > See "ª" above for explanation of what's going on here w/ the string check.
                if (_.isString(err) && implSideExitHandlerCbs[err]) {
                  implSideExitHandlerCbs[err]();
                }
                else {
      ...
      // Extra methods for the Deferred:
      {
        ...
        now: function (){
          ...
          try {
            ...
            if (err.code === 'E_NOT_SYNCHRONOUS' && err.traceRef === this) {
              throw flaverr({
                name:
                  'ImplementationError',
                message:
                  ...
                  'If you are the maintainer of "'+getMethodName(identity)+'", then you can change its\n'+
