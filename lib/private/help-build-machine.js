                    ...
                    if (rawOutputParsedAsErr) {
                      ...
                      if (_.isError(rawOutputParsedAsErr) && (rawOutputParsedAsErr.name === 'Exception' || (rawOutputParsedAsErr.name === 'UsageError' && rawOutputParsedAsErr.code === 'E_INVALID_ARGINS'))) {
                        ...
                        err = flaverr.wrap(
                          ...
                          }, rawOutputParsedAsErr),
                          rtOmen
                        );

                        // Note: The following approach was removed to reduce verbosity:
                        // > (but also note that it was incorporated into top-level runners
                        // > like `machine-as-action` instead)
                        // ```
                        // err = flaverr({
                        //   name: 'Error',
                        //   code: 'E_FROM_WITHIN',
                        //   message:
                        //   'Internal error occurred while running `'+getMethodName(identity)+'` at '+
                        //   flaverr.getBareTrace(rawOutputParsedAsErr, 1).replace(/^\s+at\s*/, '')+'.\n'+
                        //   '(See `.raw` for more information.)',
                        //   // rawOutputParsedAsErr.message,
