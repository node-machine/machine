 ...
 * .helpBuildMachine()
 *
 * Build a callable machine function (aka "wet machine") as a Callable using the
 * specified definition and custom usage options.
 *
 * ·····································································································
...
module.exports = function helpBuildMachine(opts, btOmen) {
  ...
  if (!getIsProductionWithoutDebug()) {
    ...
    if (opts.execStyle !== undefined && !_.contains(['deferred','immediate'], opts.execStyle)) {
      throw new Error('If specified, `execStyle` must be either "deferred" or "immediate".  But instead, got:\n'+util.inspect(opts.execStyle, {depth: 5}));
    }
  ...
  if (arginStyle === 'serial') {
    ...
    // regardless of how many inputs it has and how they're set up, and even if it doesn't define
    // an `args` array to indicate its preference for ordering.
    // (This probably just respects key order in `inputs`.)
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    var areAnyInputsRequired = _.reduce(nmDef.inputs, function(areAnyInputsRequired, inputDef) {
      return areAnyInputsRequired || inputDef.required;
    }, false);

    if (!nmDef.args && areAnyInputsRequired) {
      throw flaverr({
        name:
          'UsageError',
        message:
          'Sorry, cannot build function (`'+getMethodName(nmDef.identity)+'`) using the "serial" argin style because\n'+
          ...
          if (i !== nmDef.args.length-1){
            throw flaverr({
              ...
              message:
                ...
                '"rest" (`\'{*}\'`) notation is used, it must be the very last item in `args`.\n'+
                GENERIC_HELP_SUFFIX
            }, btOmen);
          } else {
    ...
    switch (arginStyle) {
      ...
      case 'serial':
        ...
        argins = _.reduce(arguments, function(argins, argin, i){
          if (!nmDef.args) {
            // FUTURE: use inferred ordering instead in this case, based on key order
            // (see also other "FUTURE" note above)
            throw new Error('Consistency violation: `args` is not defined in the implementation, and a suitable ordering could not be automatically inferred.');
          }
    ...
    var deferredObj = parley(
      ...
      function (done){
        ...
        switch (nmDef.implementationType) {
          ...
          case 'classical':
            if (arginStyle === 'serial' && execStyle === 'immediate') {
              // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
              // FUTURE: Support optimized execution when possible compatible usage
              // (See the bit about "run `fn`" below.  Basically we need to do that here--
              // a good chunk of the rest can be skipped, except for validation/coercion/meta etc.)
              //
              // > Note that this should still check whether or not `fn` is an `async function` and warn/error accordingly.
              // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
              return done(flaverr({name:'UsageError', message: 'The experimental `classical` implementation type is not fully supported yet.  See https://github.com/node-machine/machine/commits/c0d7dba572018a7ec8d1d0683abb7c46f0aabae8 and https://github.com/node-machine/spec/pull/2/files#diff-eba3c42d87dad8fb42b4080df85facecR95 for background, or https://sailsjs.com/support for help.'}, rtOmen));
            } else {
              // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
              // FUTURE: Support automatically mapping this "classical" implementation type to any usage:
              // (see c0d7dba572018a7ec8d1d0683abb7c46f0aabae8 and https://github.com/node-machine/spec/pull/2/files#diff-eba3c42d87dad8fb42b4080df85facecR95)
              //
              // > Note that this should still check whether or not `fn` is an `async function` and warn/error accordingly.
              // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
              return done(flaverr({name:'UsageError', message: 'The experimental `classical` implementation type is not fully supported yet.  See https://github.com/node-machine/machine/commits/c0d7dba572018a7ec8d1d0683abb7c46f0aabae8 and https://github.com/node-machine/spec/pull/2/files#diff-eba3c42d87dad8fb42b4080df85facecR95 for background, or https://sailsjs.com/support for help.'}, rtOmen));
                  ...
                  if (undefined === rawOutput) {
                    ...
                    if (rawOutputParsedAsErr) {
                      ...
                      if (_.isError(rawOutputParsedAsErr) && (rawOutputParsedAsErr.name === 'Exception' || (rawOutputParsedAsErr.name === 'UsageError' && rawOutputParsedAsErr.code === 'E_INVALID_ARGINS'))) {
                        ...
                        // IWMIH, it means this error is from some other, unrelated machine called internally
                        // from within the implementation.  So we can wrap it up in an external error to ease
                        // negotiation in userland code (which should _never_ need to worry about tracerefs!)
                        ...
                        err = flaverr({
                          name: 'Error',
                          message: 'Internal error occurred while running `'+getMethodName(identity)+'`.  '+rawOutputParsedAsErr.message,
                        ...
                        var props = {
                          ...
                          message: (function _gettingErrMsg(){
                            ...
                            if (nmDef.exits[miscExitCodeName].description) {
                              // errMsg += '\n║ «'+(nmDef.exits[miscExitCodeName].friendlyName || _.words(miscExitCodeName).join(' '))+'»';
                              // errMsg += '\n╚ '+nmDef.exits[miscExitCodeName].description+'';
                              // errMsg += '\n'+nmDef.exits[miscExitCodeName].description+'';
                            ...
                            if (rawOutput !== undefined) {
                              // errMsg += '\n'+(nmDef.exits[miscExitCodeName].outputFriendlyName||rttc.getDisplayTypeLabel(nmDef.exits[miscExitCodeName].outputType))+':';
                              // errMsg += '\n';
                              ...
                              if (rawOutputParsedAsError) {
                                ...
                                // errMsg += '\nAdditional data:';
                                // errMsg += '  '+((rawOutputParsedAsError.toString&&rawOutputParsedAsError.toString())||rawOutputParsedAsError.message||rawOutputParsedAsError.name)+')';
                                // errMsg += '\n('+((rawOutputParsedAsError.toString&&rawOutputParsedAsError.toString())||rawOutputParsedAsError.message||rawOutputParsedAsError.name)+')';
                                // errMsg += '\nAdditional data:';
                              ...
                              // // If the raw output is an Error instance (or close enough), then just append _its_ message
                              // if (rawOutputParsedAsError) {
                              //   errMsg += '\n  ...another Error (see `.raw`).';
                              //   // errMsg += ' ('+((rawOutputParsedAsError.toString&&rawOutputParsedAsError.toString())||rawOutputParsedAsError.message||rawOutputParsedAsError.name)+')';
                              //   // errMsg += '\n\n«'+((rawOutputParsedAsError.toString&&rawOutputParsedAsError.toString())||rawOutputParsedAsError.message||rawOutputParsedAsError.name)+'»';
                              //   // errMsg += '\n '+(rawOutputParsedAsError.message||rawOutputParsedAsError.name);
                              // } else {
                              //   errMsg += '\n\n'+util.inspect(rawOutput, {depth: 5});
                              // }
                            }
                            // errMsg += '\n [?] Stuck?  Try https://sailsjs.com/support';

                            // // If there was no additional information _at all_, then include a bit more.
                            // if (!nmDef.exits[miscExitCodeName].description && rawOutput === undefined) {

                            // }

                            // var errMsg = '`'+getMethodName(identity)+'` failed.\n';
                            // if (!nmDef.exits[miscExitCodeName].description) {
                            //   errMsg += '╚ «'+miscExitCodeName+'»\n';
                            // } else {
                            //   errMsg += '║ «'+miscExitCodeName+'»\n';
                            //   errMsg += '╚ '+nmDef.exits[miscExitCodeName].description+'\n';
                            // }
                            // if (nmDef.exits[miscExitCodeName].outputType) {
                            //   errMsg += ''+(nmDef.exits[miscExitCodeName].outputFriendlyName||rttc.getNounPhrase(nmDef.exits[miscExitCodeName].outputType))+':\n';
                            //   errMsg += '\n';
                            //   errMsg += util.inspect(result, {depth: 5});
                            // }
                            ...
                            // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
                            // Note:
                            // The error message for Exceptions is tricky, because we want to use as much metadata
                            // as possible, but we must also be careful not to squish any important underlying
                            // information.
                            //
                            // Here's an example of a few different ways one such hypothetical message might look,
                            // when it's actually all printed out:
                            //
                            // ```
                            //   Exception: `template` failed («missingData»).
                            // ```
                            // ```
                            ...
                            // ```
                            // ```
                            //   Exception: `read-json` failed ("isDirectory").
                            //   ║ «Is directory»
                            //   ╚ The specified path pointed to a directory.
                            //
                            // ```
                            // ```
                            //   Exception: Failed with "conflict" at `enroll-student`.  Specified user is already enrolled in one or more of those courses.
                            //
                            //   Conflicting courses:
                            //
                            //   ['M403', 'E301K']
                            // ```
                            // ```
                            //   Exception: During `enroll-student`  Specified user is already enrolled in one or more of those courses.
                            //   Exit: "conflict"
                            //
                            //   Conflicting courses:
                            //
                            ...
                            // ```
                            // ```
                            //   Exception: Failed to `enroll-student`.  Specified user is already enrolled in one or more of those courses.
                            //     (`enroll-student`
                            //   triggered its "conflict" exit.)
                            //   Conflicting courses:
                            //
                            //   ['M403', 'E301K']
                            // ```
                            // ```
                            //   Exception: Failed to `enroll-student`.  (Exit: "conflict")
                            //   Data:
                            //
                            //   ['M403', 'E301K']
                            // ```
                            // ```
                            //   `enroll` triggered its "conflict" exit.
                            //   Also got data:
                            //
                            //   ['M403', 'E301K']
                            // ```
                            // ```
                            //   Specified user is already enrolled in one or more of those courses.  (`enroll` triggered its "conflict" exit.)
                            //   Also got results:
                            //
                            //     Error: Could not read property "courses" of undefined
                            //       at dagadghdgsdsghadsg 9:33
                            //       asdgkdsadgsdgsdgs 19:834
                            //       …
                            // ```
                            // ^^(i.e. exit description, if there is one, followed by paranthetical explanation,
                            //    and outputfriendlyname +":" (or "Data:") on the next line, followed by pretty-printed
                            //    result data.  If no output was expected, but we got some anyway, then use "Got unexpected data:".
                            // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
                            ...
                            // var errMsg = '`'+getMethodName(identity)+'` triggered its `'+miscExitCodeName+'` exit';
                            ...
                            // }
                            // else {
                            //   var resultParsedAsErr = flaverr.parseError(result);

                            //   //  • if the raw output is an Error instance (or close enough), then just append _its_ message
                            //   if (resultParsedAsErr) {
                            //     errMsg += ': '+(resultParsedAsErr.message||resultParsedAsErr.name);
                            //     // errMsg += ' with Error: '+(resultParsedAsErr.message||resultParsedAsErr.name);
                            //   }
                            ...
                            // // Build base failure msg (used as prefix below)
                            // var baseFailureMsg = (function _getImperativeMoodFragmentWithEndingPunctuation(){
                            // // var baseFailureMsg = 'Something went wrong trying to '+(function _getImperativeMoodFragmentWithEndingPunctuation(){
                            // // var baseFailureMsg = 'Could not run '+(function _getImperativeMoodFragmentWithEndingPunctuation(){
                            //   var imperativeMoodFragmentWithEndingPunctuation;
                            //   if (nmDef.description) {
                            //     imperativeMoodFragmentWithEndingPunctuation = 'Failed trying to '+nmDef.description[0].toLowerCase() + nmDef.description.slice(1);// + '.';
                            //     // imperativeMoodFragmentWithEndingPunctuation = 'Could not '+nmDef.description[0].toLowerCase() + nmDef.description.slice(1);// + '.';
                            //     // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
                            ...
                            //     // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
                            //   }
                            //   // else if (nmDef.friendlyName) {
                            //   //   imperativeMoodFragmentWithEndingPunctuation = nmDef.friendlyName[0].toLowerCase() + nmDef.friendlyName.slice(1);
                            //   // }
                            //   else {
                            //     imperativeMoodFragmentWithEndingPunctuation = '`'+getMethodName(identity)+'` triggered its "'+miscExitCodeName+'" exit.';
                            //     // imperativeMoodFragmentWithEndingPunctuation = '`'+getMethodName(identity)+'()` failed with `'+miscExitCodeName+'`.';
                            //     // imperativeMoodFragmentWithEndingPunctuation = 'run `'+getMethodName(identity)+'`.';
                            //   }
                            ...
                            // var errMsg = baseFailureMsg;

                            // // And is then augmented by some additional basic rules:
                            // // errMsg += '  Something went wrong.  `'+getMethodName(identity)+'` triggered its `'+miscExitCodeName+'` exit';
                            // // //  • if there was no raw output, append the exit description if available.
                            // // if (result === undefined) {
                            // //   if (nmDef.exits[miscExitCodeName].description) {
                            // //     errMsg += ': '+nmDef.exits[miscExitCodeName].description;
                            ...
                            // // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
                            // // TODO: try always including exit description, if available-- like:
                            // // ```
                            // errMsg += '  '+(
                            //   nmDef.exits[miscExitCodeName].description?
                            //   '('+nmDef.exits[miscExitCodeName].description+')'
                            //   :
                            //   ''
                            //   // '(Reason: "'+miscExitCodeName+'")'
                            //   // '(Triggered its `'+miscExitCodeName+'` exit.)'
                            ...
                            // > Note that the old methodology is still preferable in some ways, because triggering an exit _doesn't necessarily_ mean there
                            // > was an error. But let's real-talk for a second:  99.9% of the time it does.  And it sucks to stutter out a less clear message
                            // > that hurts the overall developer experience for the vast majority of use cases just because there's a TINY chance it doesn't
                            // > apply.   Hence why we've experimented with changing it (see above.)  The original way is included below, with various inline
                            // > changes for now  (it's really hard to change this particular error message and still have error messages make sense generically).
                            // ```
                            // The error msg always begins with a standard prefix:
                            // var errMsg = '`'+getMethodName(identity)+'` came back "'+miscExitCodeName+'"';
                            // var errMsg = '`'+getMethodName(identity)+'` exited "'+miscExitCodeName+'"';
                            // var errMsg = '`'+getMethodName(identity)+'` triggered "'+miscExitCodeName+'"';
                            // var errMsg = '`'+getMethodName(identity)+'` failed ("'+miscExitCodeName+'")';
                            // var errMsg = '`'+getMethodName(identity)+'` triggered its "'+miscExitCodeName+'" exit';
                            // var errMsg = '`'+getMethodName(identity)+'` triggered its `'+miscExitCodeName+'` exit';
                            // // And is then augmented by some additional basic rules:
                          ...
                          //     output: rawOutput,
                          // ```
                          // ^^^ but at this point, probably not.
                          // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
                        };
      ...
      (
        _.reduce(nmDef.inputs, function (memo, inputDef, inputCodeName){
          ...
          var pad = _.repeat(' ', Math.max(
            MIN_PADDING,
            lengthOfLongestInputCodeName-inputCodeName.length+MIN_PADDING
          ));
  ...
  });//…)   ( defining property on wet machine )
