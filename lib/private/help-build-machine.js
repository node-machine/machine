  ...
  var wetMachine = function runFn(_argins, _explicitCbMaybe, _metadata){
    ...
    var deferredObj = parley(
      ...
      // Extra methods for the Deferred:
      {
         ...
         * > To test on the REPL:
         * > ```
         * > require('./').build({identity:'asdf', timeout: 500, sideEffects: 'idempotent', inputs:{foo: {type:'number', required: true}},fn: function(inputs, exits){ if (inputs.foo+Math.random() > 0.1) {  setTimeout(()=>{ exits.success(); }, 1000); } else  { exits.success(); } } })({foo: 0}).retry({name:'TimeoutError'}, [5, 10, 5000]).timeout(550).log()
         * > ```
         *
        ...
        retry: function (negotiationRuleOrRetryDelaySeries, specificRetryDelaySeries){
          ...
          this.tolerate(negotiationRule, function(originalErr){
            ...
            var originalInvocationTimeout = originalDeferred._timeout;
            // TODO: use public API to determine this ^^
            // console.log('originalInvocationTimeout', originalInvocationTimeout);
            return new Promise(function(resolve, reject){
              ...
              (function _recursively(done){
                // console.warn('Waiting '+retryDelaySeries[numRetriesSoFar - 1]+'ms before trying again...');
                setTimeout(function (){
                  ...
                  if (originalInvocationTimeout) {
                    π.timeout(originalInvocationTimeout);
                  }
