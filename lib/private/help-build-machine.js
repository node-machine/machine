  ...
  var wetMachine = function runFn(_argins, _explicitCbMaybe, _metadata){
    ...
    var deferredObj = parley(
      ...
      function (done){
        ...
        switch (nmDef.implementationType) {
          ...
          case 'analog':
            ...
            // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
            // FUTURE: If `fn` is expecting only one argument (i.e. no `exits`), then
      ...
      // Extra methods for the Deferred:
      {
        ...
        // To test on the REPL:
        // require('./').build({identity:'asdf', timeout: 500, sideEffects: 'idempotent', inputs:{foo: {type:'number', required: true}},fn: function(inputs, exits){ if (inputs.foo+Math.random() > 0.5) {  setTimeout(()=>{ exits.success(); }, 1000); } else  { exits.success(); } } })({foo: 0}).retry({name:'TimeoutError'}).log()
        retry: function (negotiationRuleOrRetryDelaySeries, specificRetryDelaySeries){
          var negotiationRule = negotiationRuleOrRetryDelaySeries;
          var retryDelaySeries = specificRetryDelaySeries;
          // TODO handle variadic usage


          if (retryDelaySeries === undefined) {
            retryDelaySeries = [ 100, 200, 400 ];
          } else if (!_.isArray(retryDelaySeries)) {
            throw new Error('TODO: this usage error');
          } else if (retryDelaySeries.length < 1) {
            throw new Error('TODO: this usage error');
          } else if (!_.all(retryDelaySeries, function(ms){ return _.isNumber(ms) && ms > 0 && Math.floor(ms) === ms; })) {
            throw new Error('TODO: this usage error');
          }

          var def = wetMachine.getDef();
          if (def.sideEffects !== 'idempotent' && def.sideEffects !== 'cacheable') {
            throw new Error('TODO: this usage error (not idempotent enough!)');
          }
          ...
          // TODO: in parley: make .tolerate() support throwing things straight through  (only .tolerate() though! too risky for .intercept())
          // TODO: in parley & here: a way to somehow signal async function w/o breaking node≤7 compat.

          this.tolerate(negotiationRule, async function(originalErr){
            var numRetriesSoFar = 1;
            var errorsThatCausedRetries = [ originalErr ];
            return new Promise(function(resolve, reject){
              (function _recursively(done){
                console.log(`waiting ${retryDelaySeries[numRetriesSoFar - 1]}ms before trying again`);
                setTimeout(function (){
                  // TODO: also apply invocation (i.e. userland) options  (e.g. consider `.timeout()`)
                  // TODO: verify this works with default argins
                  callableWithNormalizedUsage(_.clone(argins), undefined, metadata)
                  .tolerate(negotiationRule, async function(err) {
                    return new Promise(function(resolve, reject){
                      errorsThatCausedRetries.push(err);
                      if (numRetriesSoFar === retryDelaySeries.length) {
                        // TODO: figure out a way to prevent pieces of the generic error msg from getting repeated a bajillion times
                        reject(new Error('TODO: this "too many retries" error - be sure to include `errorsThatCausedRetries`'));
                      } else {
                        numRetriesSoFar++;
                  ...
                  .exec(done);//_∏_  </ callableWithNormalizedUsage().exec() >
              ...
              // ~∞%°
                ...
                if (err) {
                  ...
                  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
                  // FUTURE: consider an opt-in warning here that logs the retries
                  // (or better yet, a configurable notifier function)
                  // ```
                  // console.warn('blah blah', errorsThatCausedRetries);
                  // ```
                  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
                  resolve(result);
                }
