...
module.exports = function helpBuildMachine(opts, btOmen) {
  ...
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  // TODO: Decide whether to bring `forceCoerceAndClone` or `coerceAndCloneOrError` back by default.
  // (Or maybe just to leave it as `doNotCheck`...)
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
              ...
              (function _attachingHandlerCbs(proceed){
                ...
                handlerCbs.error = function(rawOutput){
                  ...
                  if (undefined === rawOutput) {
                    ...
                    err = rawOutput;
                    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
                    // FUTURE: ^Instead of the above, consider this:  Imagine we checked the Error instance
                    // to see if it has a `code` or `name` that indicates any special meaning -- whether that
                    // be a `name` of `RuntimeValidationError`/ `TimeoutError`, or just a `code` that happens
                    // to overlap with the name of one of this machine's declared exits.  In either of these
                    // cases, we might strip that information off and preserve it instead.
                    //
                    // For example:
                    // ```
                    // if (_.isError(rawOutput) && rawOutput.name === 'Exception') {
                    //   if (rawOutput._traceRef === traceRef) { throw new Error('Consistency violation: Somehow received exit Exception for this invocation, but in the internal handler for the error exit!  This should never happen!  (There is probably a bug in this version of the runner-- please file a bug at https://sailsjs.com/bugs)'); }
                    //
                    ...
                    //   err = flaverr({
                    //     message: 'Internal error occurred while running `'+getMethodName(identity)+'`.  '+rawOutput.message,
                    //     raw: rawOutput
                    //   }, rtOmen);
                    // }
                    // else {
                      ...
                      try {
                        ...
                        switch (err.code) {
                          case 'E_INVALID':
                            throw flaverr({
                              name: 'ImplementationError',
                              code: 'E_UNEXPECTED_OUTPUT',
                              message: 'Successfully ran "'+getMethodName(identity)+'", but with an unexpected result.\n'+
                              // ^^TODO: expand this error message to explain why (see what's in normalize-argins)
                              '\n'+
                              'If you are the maintainer of "'+getMethodName(identity)+'", then you can change '+
                              'its implementation to either send back the proper expected output, or '+
                              'update the exit definition to clarify the expectation.  Otherwise, please '+
                              'file a bug report with the maintainer, or fork your own copy and fix that.\n'+
                              GENERIC_HELP_SUFFIX,
                              raw: rawOutput
                            }, rtOmen);
                          default:
                    ...
                    try {
                      if (resultValidationTactic === 'doNotCheck' || !nmDef.exits[miscExitCodeName].outputType) {
                        ...
                        try {
                          ...
                          throw flaverr({
                            name: 'Error',
                            message: 'After running "'+getMethodName(identity)+'", which triggered its "'+miscExitCodeName+'" exit, an unexpected error was encountered while attempting to coerce the resulting exception.  Internal error details: '+util.inspect(err, {depth: 5}),
                            raw: err,
                            outputThatCouldNotBeCoerced: rawOutput
                          }, rtOmen);
                        }
                          ...
                          switch (err.code) {
                            case 'E_INVALID':
                              throw flaverr({
                                name: 'ImplementationError',
                                code: 'E_UNEXPECTED_OUTPUT',
                                message: 'Ran `'+getMethodName(identity)+'`, which triggered its "'+miscExitCodeName+'" exit, but got an unexpected result.\n'+
                                // ^^TODO: expand this error message to explain why (see what's in normalize-argins)
                                '\n'+
                                'If you are the maintainer of "'+getMethodName(identity)+'", then you can change '+
                                'its implementation to either send back the proper expected output, or '+
                    ...
                    var err = flaverr({
                      ...
                      message: (function _gettingErrMsg(){
                        ...
                        //  • if the raw output is an Error instance, then just append _its_ message
                          ...
                          errMsg += ': '+(result.message||result.name);
                        }
                        ...
                        else if (_.isObject(result) && result.cause && _.isError(result.cause)) {
                          errMsg += ': '+result.cause.message;
                        }
      ...
      // Extra methods for the Deferred:
      {
        ...
        execSync: function (){
          ...
          if (!isFinishedExecuting) {
            ...
            // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
            // FUTURE: use some kind of mechanism from parley to indicate
            // this Deferred should now silently ignore any attempts to
            // trigger its callbacks- whether that's from within the custom
            // implementation or from something more built-in (e.g. the timeout)
            // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
          }//•
