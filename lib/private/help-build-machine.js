                ...
                handlerCbs.success = function(rawOutput){
                  ...
                  if (resultValidationTactic === 'doNotCheck' || !nmDef.exits.success.outputType) {
                    ...
                    try {
                      ...
                      throw flaverr({
                        message: 'Consistency violation: Unexpected error when attempting to coerce the result from successfully running `'+getMethodName(identity)+'`.  Error details: '+util.inspect(err, {depth: 5}),
                        raw: err,
                        outputThatCouldNotBeCoerced: rawOutput,
                      }, rtOmen);
                    }
                  }
                  else if (resultValidationTactic === 'coerceAndCloneOrError') {
                    try {
                      result = rttc.validate(nmDef.exits.success.outputType, rawOutput);
                    } catch (err) {
                      throw err;// TODO: better error message
                    }
                  }
                  else if (resultValidationTactic === 'error') {
                    try {
                      result = rttc.validateStrict(nmDef.exits.success.outputType, rawOutput);
                    } catch (err) {
                      throw err;// TODO: better error message
                    }
                  }//ﬁ
                    ...
                    if (resultValidationTactic === 'doNotCheck' || !nmDef.exits[miscExitCodeName].outputType) {
                      ...
                      try {
                        ...
                        try {
                          ...
                          throw flaverr({
                            message: 'Consistency violation: After running `'+getMethodName(identity)+'`, which triggered its '+miscExitCodeName+' exit, encountered an unexpected error when attempting to coerce the resulting exception.  Error details: '+util.inspect(err, {depth: 5}),
                            raw: err,
                            outputThatCouldNotBeCoerced: rawOutput
                          }, rtOmen);
                        }
                      ...
                      try {
                        result = rttc.validateStrict(nmDef.exits[miscExitCodeName].outputType, rawOutput);
                      } catch (err) {
                        throw err;// TODO: better error message
                      }
                    }//ﬁ
                    ...
                    var err = flaverr({
                      ...
                      message: (function _gettingErrMsg(){
                        ...
                        //  • if the raw output is an Error instance, then just append _its_ message
                          ...
                          errMsg += ': '+(rawOutput.message||rawOutput.name);
                        }
