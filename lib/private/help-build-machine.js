    ...
    var deferredObj = parley(
      ...
      // Extra methods for the Deferred:
      {
        ...
        retry: function (negotiationRuleOrRetryDelaySeries, specificRetryDelaySeries){
          ...
          var negotiationRule;
          var retryDelaySeries;
          if (_.isArray(negotiationRuleOrRetryDelaySeries) && specificRetryDelaySeries === undefined) {
            retryDelaySeries = negotiationRuleOrRetryDelaySeries;
          }
          else {
            negotiationRule = negotiationRuleOrRetryDelaySeries;
            retryDelaySeries = specificRetryDelaySeries;
          }
          ...
          this.tolerate(negotiationRule, function(originalErr){
            ...
            return new Promise(function(resolve, reject){
              (function _recursively(done){
                ...
                setTimeout(function (){
                  ...
                  .tolerate(negotiationRule, function(err) {
                    return new Promise(function(resolve, reject){
                      ...
                      if (numRetriesSoFar === retryDelaySeries.length) {
                        reject(flaverr({
                          name:
                            'UsageError',
                          message:
                            'Even after retrying '+numRetriesSoFar+' time'+(numRetriesSoFar === 1 ? '' : 's')+', this function ("'+getMethodName(def.identity)+'"),\n'+
                            'still did not return successfully.  (Check out `.raw` for details.)\n'+
                            GENERIC_HELP_SUFFIX,
                          raw:
                            errorsThatCausedRetries
                            // FUTURE: Maybe clean up output within `.raw` for known errors (e.g. timeouts)
                        }, rtOmen));
                        // ^^FUTURE: Solve the [Circular] issue so that we can bring `rtOmen` back here, instead of instantiating a new error.
                      } else {
                        numRetriesSoFar++;
