...
var getInvalidityMessage = require('./get-invalidity-message');
 ...
 * ·    ·    ·    ·    ·    ·    ·    ·    ·    ·    ·    ·    ·    ·    ·    ·    ·    ·    ·    ·    ·
 *   @property {Function?} finalAfterExec         (a special final handler function, as defined by parley)
 * ·····································································································
 * @param {Error?} btOmen
...
module.exports = function helpBuildMachine(opts, btOmen) {
  ...
  if (!getIsProductionWithoutDebug()) {
    if (!_.isObject(opts) || _.isArray(opts) || _.isFunction(opts)) {
      throw new Error('opts must be a dictionary (i.e. plain JavaScript object like `{}`).  But instead got: '+util.inspect(opts, {depth: 5}));
    }

    if (!_.isObject(opts.def) || _.isArray(opts.def) || _.isFunction(opts.def)) {
      throw new Error('`def` must be a dictionary (i.e. plain JavaScript object like `{}`).  But instead got: '+util.inspect(opts.def, {depth: 5}));
    }
    ...
    if (opts.arginValidationTactic !== undefined && !_.contains(['coerceAndCloneOrError','error','doNotCheck'], opts.arginValidationTactic)) {
      throw new Error('If specified, `arginValidationTactic` must be either "coerceAndCloneOrError", "error", or "doNotCheck".  But instead got: '+util.inspect(opts.arginValidationTactic, {depth: 5}));
    }
    if (opts.resultValidationTactic !== undefined && !_.contains(['forceCoerceAndClone','coerceAndCloneOrError','error','doNotCheck'], opts.resultValidationTactic)) {
      throw new Error('If specified, `resultValidationTactic` must be either "forceCoerceAndClone", "coerceAndCloneOrError", "error", or "doNotCheck".  But instead got: '+util.inspect(opts.resultValidationTactic, {depth: 5}));
    }
    if (opts.extraArginsTactic !== undefined && !_.contains(['warn', 'warnAndOmit','error','doNotCheck'], opts.extraArginsTactic)) {
      throw new Error('If specified, `extraArginsTactic` must be either "warn", "warnAndOmit", "error", or "doNotCheck".  But instead got: '+util.inspect(opts.extraArginsTactic, {depth: 5}));
    }
    if (opts.extraCallbacks !== undefined && !_.contains(['warnAndOmit', 'error','doNotCheck'], opts.extraCallbacks)) {
      throw new Error('If specified, `extraCallbacks` must be either "warnAndOmit", "error", or "doNotCheck" (i.e. silently omit).  But instead got: '+util.inspect(opts.extraCallbacks, {depth: 5}));
    }
    ...
    var VALID_OPTIONS = [
      ...
      // Preferences for higher-level runners:
      'finalAfterExec'
    ];
  ...
  if (arginStyle === 'serial') {
    ...
    // Or possibly it respects key order...)
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    ...
    switch (arginStyle) {
      ...
      case 'serial':
        ...
        _.each(arguments, function(argin, i){
          if (!nmDef.args) { throw new Error('Consistency violation: `args` is not defined in the implementation, and a suitable ordering could not be automatically inferred.'); }
          var supposedInputName = nmDef.args[i];
          if (!supposedInputName) { throw new Error('Invalid usage with serial arguments: Received unexpected '+(i===0?'first':i===1?'second':i===2?'third':(i+1)+'th')+' argument.'); }
      ...
      function (done){
        ...
        switch (nmDef.implementationType) {
          ...
          case 'es8AsyncFunction':
            ...
            // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
            return done(flaverr({name:'UsageError', message: 'The experimental `es8AsyncFunction` implementation type is not yet supported.  See https://github.com/node-machine/machine/commits/c0d7dba572018a7ec8d1d0683abb7c46f0aabae8 for background, or https://sailsjs.com/support for help.'}, rtOmen));
          ...
          case 'classicJsFunction':
            ...
            // > Note that this should check whether `fn` is an `async function` or not and warn/error accordingly.
            // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
            return done(flaverr({name:'UsageError', message: 'The experimental `classicJsFunction` implementation type is not yet supported.  See https://github.com/node-machine/spec/pull/2/files#diff-eba3c42d87dad8fb42b4080df85facecR95 for background, or https://sailsjs.com/support for help.'}, rtOmen));
          ...
          case undefined:
            ...
            var validationProblems = normalizeArgins(argins, arginValidationTactic, extraArginsTactic, nmDef);
              ...
              (function _attachingHandlerCbs(proceed){
                ...
                handlerCbs.error = function(rawOutput){
                  ...
                  if (undefined === rawOutput) {
                    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
                    // FUTURE: We might consider actually NOT using the omen in this case -- since we
                    // might be interested in the line of code where the `exits.error()` was called.
                    // OR, perhaps better yet, we still use the omen, but also capture an additional
                    // trace and attach it as an extra property.
                    //
                    // We could also work this stack trace info into the message itself, which might
                    // be pretty nice and turn out to be good enough-- the only issue there is that
                    // then it might be messier, and need to be truncated (which could defeat the
                    // usefulness somewhat.)
                    //
                    // (**Regardless of the approach we take for this, we might actually want to
                    // consider doing the same thing for a few of the other Errors below!**)
                    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
                    err = flaverr({
                      name: 'Error',
                    ...
                    if (rawOutputParsedAsErr) {
                      ...
                      // an Error instance).  But now, this causes a separate ImplementorError about how it
                      // should be an Error instance, just like any other non-Error data would.
                      err = flaverr({
                        name: 'ImplementationError',
                        message: 'Internal error occurred while running `'+getMethodName(identity)+'`.  Got non-Error: '+util.inspect(rawOutput, {depth: 5}),
                        raw: rawOutput
                      }, rtOmen);
                      // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
                      // FUTURE: add in additional verbiage explaining/clarifying why this ImplementationError
                      // is being displayed, and that there should always be an Error instance sent back --
                      // not some other value like this.
                      // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
                    }
                  }//ﬁ
                ...
                // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
                // FUTURE: Implement runtime refinement based on `like` and `itemOf`
                // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

                // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
                // FUTURE: Implement runtime refinement based on `getExample`
                // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
                ...
                // than the `resultValidationTactic`-- or it might not.  Not sure yet
                // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
                        ...
                        switch (err.code) {
                          case 'E_INVALID':
                            throw flaverr({
                              ...
                              '------------------------------------------------------\n'+
                              ' • '+getInvalidityMessage(nmDef.exits.success.outputType, rawOutput, err, 'result data')+'\n'+
                              '------------------------------------------------------\n'+
                              '\n'+
                          ...
                          switch (err.code) {
                            case 'E_INVALID':
                              throw flaverr({
                                ...
                                '------------------------------------------------------\n'+
                                ' • '+getInvalidityMessage(nmDef.exits[miscExitCodeName].outputType, rawOutput, err, 'result data')+'\n'+
                                '------------------------------------------------------\n'+
                                '\n'+
                                'If you are the maintainer of "'+getMethodName(identity)+'", then you can change '+
                                'its implementation accordingly (either send back the proper expected data in your '+
                      ...
                      (function _gettingExceptionPropsForFlaverr(){
                        var props = {
                          ...
                          message: (function _gettingErrMsg(){
                            ...
                            if (result === undefined) {
                              if (!_.isObject(nmDef.exits[miscExitCodeName])) { throw new Error('Consistency violation: Machine ('+getMethodName(identity)+') has become corrupted!  One of its exits (`'+miscExitCodeName+'`) has gone missing _while the machine was being executed_!'); }
            ...
            if (nmDef.fn.constructor.name === 'AsyncFunction') {
              ...
              try {
                ...
                if (_.isString(specialExitSignal) && specialExitSignal !== 'error' && implSideExitHandlerCbs[specialExitSignal]) {
                  return implSideExitHandlerCbs[specialExitSignal]();
                }
                else if (!_.isError(specialExitSignal) && !_.isArray(specialExitSignal) && !_.isFunction(specialExitSignal) && _.isObject(specialExitSignal) && _.keys(specialExitSignal).length === 1 && _.keys(specialExitSignal)[0] !== 'error' && implSideExitHandlerCbs[_.keys(specialExitSignal)[0]]) {
          ...
          this.exec(function (err, result){
            ...
            if (err) {
              ...
              if (err.name === 'Exception' && err.traceRef === privateTraceRef) {
                ...
                if (err.code === 'error') { throw new Error('Consistency violation: Recognized exceptions from the machine runner should never have a `code` property set to `error`, but this has just that!  Here is the error:'+util.inspect(err, {depth:null})); }

                // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
                // FUTURE: Consider stripping off the `.traceRef` property here, since it's
                // no longer needed and makes error console output messier.  Need to carefully
                // test this in conjunction with higher-order runners like `sails` and `whelk`
