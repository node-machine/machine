...
module.exports = function helpBuildMachine(opts, btOmen) {
  ...
  if (!btOmen && !getIsProduction()) {
    btOmen = flaverr({ name: 'Omen' }, new Error('Build-time'), helpBuildMachine);
  }//ﬁ
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  // FUTURE: Potentially provide a way to override this build-time omen behavior, irrespective
  // of production vs dev environment  (Or better yet: just the ability to use a custom build-time
  // omen from ACTUAL userland, where you call Machine.build(), instead of just being able to
  // pass it into this private utility, which doesn't actually help at all.)
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  ...
  if (implProblems.length > 0) {
    throw flaverr({
      ...
      message:
        ...
        'its underlying implementation has '+(implProblems.length===0?'a problem':implProblems.length+' problems')+':\n'+
        '------------------------------------------------------\n'+
        '• '+implProblems.join('\n• ')+'\n'+
        '------------------------------------------------------\n'+
        '\n'+
        ...
        'its implementation to solve the problem'+(implProblems.length===0?'':'s')+' above.  '+
        'Otherwise, please file a bug report with the maintainer, or fork your own copy and '+
        'fix that.'
        :
        'Please resolve the problem'+(implProblems.length===0?'':'s')+' above and try again.'
        )+'\n'+
        GENERIC_HELP_SUFFIX
    }, btOmen);
  }//•
  ...
  var wetMachine = function runFn(_argins, _explicitCbMaybe, _metadata){
    ...
    // Potentially build another "omen", now for runtime
    // (see above for explanation/reminder of what an "omen" is.)
    //
    // This allows us to provide a nice, clean, appropriate stack trace originating
    // from the line of code that actually invoked this wet machine in userland.
    var rtOmen;
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
