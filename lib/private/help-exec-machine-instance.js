...
var stripStackMessage = require('./strip-stack-message');
  ...
  (function _doCacheLookupMaybe (cb) {
    ...
    _.each(_.keys(liveMachine.exits), function (exitCodeName) {
      ...
      liveMachine._configuredExits[exitCodeName] = function __triggeredMiscExit(_resultPassedInByMachineFn){
        ...
        var errMsg = '`'+_.camelCase(liveMachine.identity)+'` triggered its `'+exitCodeName+'` exit';
        ...
        liveMachine._configuredExits.error(err_forwarding);
    ...
    try {
      ...
      try {
        ...
        // Preserve the original stack message
        var msg = e.message;

        // Split the error we got into lines, and remove the error message
        e.stack = stripStackMessage(e).split('\n');
