  ...
  var interceptedCallbacks = _.reduce(callbacks, function (memo,fn,exitName){
    ...
    memo[exitName] = function _interceptExit(value) {
      ...
      if (liveMachine._exited) {
        console.warn('In `'+liveMachine.identity+'`: attempted to call exit `'+exitName+'` after exit `'+liveMachine._exited+'` was already triggered.  If you are the maintainer of this machine, make sure your machine only ever calls one exit, exactly once!');
        return;
      }// --•
