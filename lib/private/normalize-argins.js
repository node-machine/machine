      ...
      if (arginValidationTactic === 'coerceAndCloneOrError') {
        ...
        if (!inputDef.required && argins[inputCodeName] === undefined) {
            ...
            switch (err.code) {
              ...
              case 'E_DEPRECATED_SYNTAX':
                arginProblems.push(
                  ...
                  '(a special format for concisely describing a particular data type), but it could not be parsed as such.  '+err.message
                );
                break;
              default:
                throw err;
          ...
          try {
            ...
            switch (err.code) {
              case 'E_INVALID': (function(){
                var isExpectingArrayAndGotIt = _.isArray(argins[inputCodeName])&&_.isArray(inputDef.type);
                var isExpectingDictionaryAndGotIt = _.isObject(argins[inputCodeName])&&!_.isArray(argins[inputCodeName])&&_.isObject(inputDef.type)&&!_.isArray(inputDef.type);
                arginProblems.push(
                  'Invalid "' + inputCodeName + '"'+(isExpectingArrayAndGotIt||isExpectingDictionaryAndGotIt?' '+rttc.getDisplayTypeLabel(getRdtFormattedType(inputDef.type), {capitalization: 'fragment'}):'')+':\n  · ' + (
                    _.without(_.map(err.errors, function(subErr) {
                      if (subErr.hops.length === 0) {
                        ...
                        return (subErr.actual===undefined?'Missing':'Expecting')+' '+rttc.getNounPhrase(getRdtFormattedType(subErr.expected), {determiner: 'a'})+(subErr.actual===undefined?'':', but got '+(actualTopLvlType==='json'?'some other '+rttc.getNounPhrase(actualTopLvlType,{determiner: '', capitalization: 'fragment'}):rttc.getNounPhrase(actualTopLvlType,{determiner: 'a', capitalization: 'fragment'}))+'.');
                      }
                      else {
                        ...
                        var expectedType = getRdtFormattedType(subErr.expected);
        ...
        throw new Error('Consistency violation: Unrecognized argin validation tactic (`'+arginValidationTactic+'`)');
      }//ﬁ
