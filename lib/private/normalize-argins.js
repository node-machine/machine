  ...
  if (extraArginsTactic !== 'doNotCheck') {
    ...
    if (unrecognizedInputCodeNames.length > 0) {
      ...
      if (extraArginsTactic === 'warnAndOmit') {
        ...
        console.warn('WARNING: Automatically trimmed extraneous values!  In call to '+getMethodName(nmDef.identity)+'(), '+extraArginsErrorMsg);
      }
    ...
    if (arginValidationTactic !== 'doNotCheck') {
      ...
      if (argins[inputCodeName] === undefined && !inputDef.required) {
        ...
        try {
          ...
          switch (err.code) {
            case 'E_INVALID':
              arginProblems.push(getInvalidityMessage(inputDef.type, argins[inputCodeName], err, '"'+inputCodeName+'"'));
              break;
            default:
              throw err;
          }
