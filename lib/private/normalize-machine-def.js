...
module.exports = function normalizeMachineDef(nmDef){
  ...
  for (var i=0; i<inputCodeNames; i++) {
    ...
    if (inputDef.type !== undefined) {
      ...
      // TODO: separate the "RTTC display type" from the type schema
    ...
    _.each(ANCHOR_RULES, function(ruleInfo, ruleName){
      if (inputDef[ruleName] !== undefined) {

        // var mostSpecificExpectedType = 'ref';
        // _.each(ruleInfo.expectedTypes, function(thisExpectedType) {
        //   mostSpecificExpectedType = rttc.intersection(mostSpecificExpectedType, thisExpectedType);
        //   if (_.isNull(mostSpecificExpectedType)) {
        //     throw new Error('Consistency violation: Unexpected internal error in `anchor` package.  Each of the `expectedType` arrays returned from require(\'anchor/accessible/rules\') should contain only compatible types.  But somehow, two of the ');
        //   }
        // });

        _.any(ruleInfo.expectedTypes, function(expectedType) {
          return !_.isNull(rttc.intersection(expectedType, inputDef.type));
        });
