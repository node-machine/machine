...
module.exports = function normalizeMachineDef(nmDef){
  ...
  _.each(inputCodeNames, function(inputCodeName){
    ...
    _.each(ANCHOR_RULES, function(ruleInfo, ruleName){
      ...
      if (!_.contains(ruleInfo.expectedTypes, inputDef.type)) {
        implProblems.push(inputProblemPrefix+'Cannot use `'+ruleName+'` with that `type`.');
      }
  ...
  _.each(exitCodeNames, function (exitCodeName){
    ...
    // And that's not ok:
    if (outputDeclarationStyles.length > 1 && outputDeclarationStyles.length > 1 && !(outputDeclarationStyles.length === 2 && _.contains(outputDeclarationStyles, 'outputType') && _.contains(outputDeclarationStyles, 'outputExample'))) {
