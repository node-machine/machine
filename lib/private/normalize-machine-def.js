...
module.exports = function normalizeMachineDef(nmDef, implementationSniffingTactic){
  ...
  // If `fn` is expecting only one argument (i.e. no `exits`), then instead
  // of expecting exits.success() to be called, use the act of returning
  // as the success exit signal, and use the return value as the success output.
  // implementationSniffingTactic
  // (function getWhetherLastishParamIsExits(fn) {
  //   var STRIP_COMMENTS_RX = /(\/\/.*$)|(\/\*[\s\S]*?\*\/)|(\s*=[^,\)]*(('(?:\\'|[^'\r\n])*')|("(?:\\"|[^"\r\n])*"))|(\s*=[^,\)]*))/mg;
  //   let fnStr = fn.toString().replace(STRIP_COMMENTS_RX, '');
  //   let parametersAsString = fnStr.slice(fnStr.indexOf('(')+1, fnStr.indexOf(')'));
  //   return !! (
  //     parametersAsString.match(/,\s*exits\s*$/) ||
  //     parametersAsString.match(/,\s*exits\s*,\s*(env|meta)\s*$/)
  //   );
  // })();
  // ^^^^  TODO: do this where we normalize the machine def and
  // use it to set implementationType: classical
