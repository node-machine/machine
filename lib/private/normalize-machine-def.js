...
module.exports = function normalizeMachineDef(nmDef){
  ...
  if (nmDef.identity === undefined) {
    if (nmDef.friendlyName || nmDef.description) {
      ...
      // Set a dummy `identity` to avoid breaking the very error message we use to display
      // this implementation problem later.
      nmDef.identity = 'anonymous';
    }
  } else if (!_.isString(nmDef.identity)) {
    implProblems.push('Invalid `identity` property.  (Please use a valid, non-empty string.)');
    // Again, we set a dummy `identity` here (see above for explanation).
    nmDef.identity = 'anonymous';
  }

  // Make sure the `identity` won't conflict with anything important.
  try {
  ...
  for (var i=0; i<inputCodeNames; i++) {
    ...
    if (inputDef.type !== undefined) {
      var ALLOWED_TYPES = ['string', 'number', 'boolean', 'json', 'ref'];
      if (!_.contains(ALLOWED_TYPES, inputDef.type)){
        implProblems.push('Unrecognized `type`.  (Must be \'string\', \'number\', \'boolean\', \'json\' or \'ref\'.)');
      }//ﬁ
      ...
      // (It must be an RTTC exemplar which can be used to derive a `type`.)
      // TODO
    } else {
    ...
    if (inputDef.isExemplar !== undefined){
      if (!_.isBoolean(inputDef.isExemplar)){
        implProblems.push('Invalid `isExemplar` property.  (If specified, must be boolean: `true` or `false`.)');
      }
    }
    ...
    if (inputDef.required !== undefined){
      if (!_.isBoolean(inputDef.required)){
        implProblems.push('Invalid `required` property.  (If specified, must be boolean: `true` or `false`.)');
      }
    }
    ...
    if (inputDef.allowNull !== undefined){
      if (!_.isBoolean(inputDef.allowNull)){
        implProblems.push('Invalid `allowNull` property.  (If specified, must be boolean: `true` or `false`.)');
      }
      else if (inputDef.required === true) {
        implProblems.push('Defined with both `allowNull: true` and `required: true`... but that wouldn\'t make any sense.  (These settings are mutually exclusive.)');
      }
      else if (inputDef.type !== 'json' && inputDef.type !== 'ref') {
        implProblems.push(
        'Defined with `allowNull: true`, which is unnecessary in conjunction with this input\'s '+
        'declared `type`, "'+inputDef.type+'".  (With this type restriction, `null` would never '+
        'be valid anyway.)');
      }
    }
