...
module.exports = function normalizeMachineDef(nmDef){
  ...
  if (nmDef.implementationType !== undefined) {
    if (!_.isString(nmDef.implementationType) || (!nmDef.implementationType.match(/^string\:.+$/) && (nmDef.implementationType !== 'abstract' && nmDef.implementationType !== 'composite' && nmDef.implementationType !== 'classical' && nmDef.implementationType !== 'analog' && nmDef.implementationType !== ''))) {
      implProblems.push('Invalid `implementationType` property.  (If specified, must be \'abstract\', \'composite\', \'classical\', \'analog\', or a code string+language declaration like \'string:js\' or \'string:c\'.)');
    }
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    // FUTURE: If abstract, ensure there is no `fn`  (and then make everything else work given that)
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    // FUTURE: If `string:*`, etc. ensure that a code string is supplied instead of an `fn`
    // (and then make everything else work given that)
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  }
  ...
  _.each(inputCodeNames, function(inputCodeName){
    ...
    if (inputDef.type !== undefined) {
      ...
      implProblems.push(inputProblemPrefix+'Must have `type`, or at least an `example`.  (If you aren\'t sure what to use, just go with `type: \'ref\'.)');
    }//ﬁ
    ...
    // (Note that, at this point, we know we have a valid `type`.)
    _.each(ANCHOR_RULES, function(ruleInfo, ruleName){
      if (inputDef[ruleName] === undefined) { return; }
      ...
      if (!_.contains(ruleInfo.expectedTypes, inputDef.type)) {
  ...
  _.each(exitCodeNames, function (exitCodeName){
    ...
    // Otherwise the basic structure makes sense, so we'll dive in a bit deeper:
      ...
      if (exitDef.outputType !== undefined) {
        ...
        } else if (declarationStyle === 'itemOf') {
