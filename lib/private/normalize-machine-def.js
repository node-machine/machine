...
module.exports = function normalizeMachineDef(nmDef){
  ...
  if (!nmDef.identity) {
    implProblems.push(
      'Could not infer an identity.  (Definition is missing `friendlyName` in your definition.)'
    );
  }
  ...
  if (nmDef.fn===undefined) {
    implProblems.push('Definition is missing the `fn` property.');
  }
    ...
    throw flaverr({
      ...
      message:
        ...
        'Definition has invalid `fn` property.  (Please use a valid JavaScript function.)\n'+
        GENERIC_HELP_SUFFIX
    }, omen);
  }
  ...
  if (nmDef.timeout!==undefined){
    if (!_.isNumber(nmDef.timeout)){
      throw flaverr({
        ...
        message:
          'Could not interpret "'+methodName+'".\n'+
          'Definition has invalid `timeout` property.  (Must be a number greater than zero.)\n'+
          GENERIC_HELP_SUFFIX
      }, omen);
    }//•
  }//ﬁ
