...
module.exports = function normalizeMachineDef(nmDef){
  ...
  _.each(inputCodeNames, function(inputCodeName){
    ...
    if (inputDef.defaultsto !== undefined){
      implProblems.push(inputProblemPrefix+'Did you mean `defaultsTo`? (camelcase, with a capital "T")');
    }

    // `allownull` (lowercase)
    if (inputDef.allownull !== undefined){
      implProblems.push(inputProblemPrefix+'Did you mean `allowNull`? (camelcase, with a capital "N")');
    }
