...
module.exports = function normalizeMachineDef(nmDef){
  ...
  if (nmDef.implementationType !== undefined) {
    if (!_.isString(nmDef.implementationType) || (!nmDef.implementationType.match(/^string\:.+$/) && (nmDef.implementationType !== 'abstract' && nmDef.implementationType !== 'composite' && nmDef.implementationType !== 'classic' && nmDef.implementationType !== 'analog' && nmDef.implementationType !== ''))) {
      implProblems.push('Invalid `implementationType` property.  (If specified, must be \'abstract\', \'composite\', \'classic\', \'analog\', or a code string+language declaration like \'string:js\' or \'string:c\'.)');
    }
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
