 ...
 *
 * Validate a dictionary of argins.
 *
 * @param  {Machine} machine  - a live machine instance
 *
 * @returns {Dictionary}
...
module.exports = function validateConfiguredInputValues(machine){
  ...
  _.each(machine.inputs, function (inputDef, inputName){
    ...
    if (inputDef.required){
      if (_.isUndefined(machine._configuredInputs[inputName])){
        errors.push(
          flaverr({
            ...
            input: inputName,
            reason: '`'+inputName+'` is a required input- but it was not defined.'
          }, new Error('`'+inputName+'` is a required input- but it was not defined.'))
      ...
      if (_.isString(inputDef.example) && inputDef.example !== '*' && inputDef.example !== '===' && machine._configuredInputs[inputName] === '') {
        errors.push(
          flaverr({
            code: 'E_INPUT_REQUIRED',
            input: inputName,
            reason: '`'+inputName+'` is a required string input- but an empty string was supplied.'
          }, new Error('`'+inputName+'` is a required input- but an empty string was supplied.'))
          // TODO: ^^investigate making this a simple dictionary (doesn't really need to be an Error instance,
          //  since it'll get wrapped up in another one anyway.  Remember: stack access is very slow.)
        );
        return;
      ...
      if (_.isNull(machine._configuredInputs[inputName]) && (inputDef.example === '*' || inputDef.example === '===')) {
        errors.push(
          flaverr({
            code: 'E_INPUT_REQUIRED',
            input: inputName,
            reason: '`'+inputName+'` is a required input- but `null` was supplied.'
          }, new Error('`'+inputName+'` is a required input- but `null` was supplied.'))
          // TODO: ^^investigate making this a simple dictionary (doesn't really need to be an Error instance,
          //  since it'll get wrapped up in another one anyway.  Remember: stack access is very slow.)
        );
        return;
    ...
    // If a null value is provided and `allowNull` is `true`, don't validate.
    if (inputDef.allowNull === true && _.isNull(machine._configuredInputs[inputName])) {
      coercedInputValues[inputName] = null;
      return;
    }
...
function getExample (inputDef, inputName, allConfiguredInputValues, machineDef) {
  ...
  //
  // ====================================================================================
  // > TODO: Deprecate `like` in top-level machine input definitions.
  // > It should only be used in top-lvl exit defs, or in contract input/exit defs.
  // ====================================================================================
  ...
  //
  // ====================================================================================
  // > TODO: Deprecate `itemOf` in top-level machine input definitions.
  // > It should only be used in top-lvl exit defs, or in contract input/exit defs.
  // ====================================================================================
  ...
  // If `getExample()` was not provided...
  if (!inputDef.getExample){

    // Handle explicit example (just return it).
    return inputDef.example;
  }

  // Otherwise, handle getExample():
  //
  // ====================================================================================
  // > TODO: Deprecate `getExample` in top-level machine input definitions.
  // > It should only be used in top-lvl exit defs.
  // ====================================================================================
  ...
  var env = {
    _: _,
    rttc: rttc,
    types: rttc,//<< for backwards compat.
  };
  try {
