...
module.exports = function verifyExitDefinition(defToResolve, machineDefinition){
  ...
  if (!_.isUndefined(defToResolve.like)) {
    ...
    if (!referencedInput) {
      ...
      }, new Error('Failed to instantiate machine ("'+_.camelCase(machineDefinition.identity)+'") from the specified machine definition.\n`like` should refer to a known machine input, but there is no input named `'+defToResolve.like+'`.'));
    }//>-•
    ...
    if (_.isUndefined(referencedInput.example)) {
      ...
      throw flaverr({
        code: 'UNRECOGNIZED_INPUT',
        machine: machineDefinition.identity
      }, new Error('Failed to instantiate machine ("'+_.camelCase(machineDefinition.identity)+'") from the specified machine definition.\n`like` should refer to a machine input with an explicit example (i.e. it can\'t use `like` or `itemOf` too!)  The referenced input (named `'+defToResolve.like+'`) does not have an explicit example.'));
    }//--•
  ...
  // If `itemOf` was provided, return the pattern of the array example from the specified input.
    ...
    if (!referencedInput) {
      throw flaverr({
        code: 'UNRECOGNIZED_INPUT',
        machine: machineDefinition.identity
      }, new Error('Failed to instantiate machine ("'+_.camelCase(machineDefinition.identity)+'") from the specified machine definition.\n`itemOf` should refer to a known machine input, but there is no input named `'+defToResolve.itemOf+'`'));
    }//--•
    ...
    if (_.isUndefined(referencedInput.example)) {
      throw flaverr({
        ...
        'Failed to instantiate machine (`'+_.camelCase(machineDefinition.identity)+'`) from the specified machine definition.\n`itemOf` should refer to a machine input with an explicit example (i.e. it can\'t use `like` or `itemOf` too!)  The referenced input (named `'+defToResolve.itemOf+'`) does not have an explicit example.')
      );
    }//--•
    ...
    if (!_.isArray(referencedInput.example)) {
      throw flaverr({
        ...
        'Failed to instantiate machine (`'+_.camelCase(machineDefinition.identity)+'`) from the specified machine definition.\n`itemOf` should refer to a machine input with a patterned array example, but the example of input `'+defToResolve.itemOf+'` is: '+util.inspect(referencedInput.example, {depth: null})
      ));
    }//--•
    ...
    if (referencedInput.length < 1) {
      throw flaverr({
        ...
        'Failed to instantiate machine (`'+_.camelCase(machineDefinition.identity)+'`) from the specified machine definition.\n`itemOf` should refer to a machine input with a patterned array example, but instead, the example of input `'+defToResolve.itemOf+'` was `[]`!  To indicate an array with JSON-compatible contents, use `[\'*\']`.  Or to indicate an array of _ANYTHING_, use `[\'===\']`.'
      ));
    }//--•
