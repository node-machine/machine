...
function getExample (inputDef, inputName, allConfiguredInputValues) {
  ...
  if (_.isString(inputDef.like)) {
    ...
    return rttc.intersection(inputDef.example, allConfiguredInputValues[inputDef.like], true, true);
  }
  ...
  if (_.isString(inputDef.itemOf)) {
    ...
    //  statisfy BOTH schemas-- i.e. the intersection.  This MAY NOT always work
    //  because, at this point, input validation has not run yet.)
    return rttc.intersection(inputDef.example, otherArrayInputVal[0], true, true);
  }
