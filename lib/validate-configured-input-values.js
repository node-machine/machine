    ...
    if (!_.isUndefined(inputDef.contract)) {
      if (typeSchema === 'lamda') {
        try {
          ...
          // If lamda input def specifies a `defaultsTo`, and no input value was provided, go ahead and instantiate it into a machine.
          // (TODO: see if we can move the contract stuff AFTER the main defaultsTo logic over in `Machine.prototype.exec.js`--
          // only reason it's here is because it was added before rttc supported the lamda type).
          if (!_.isUndefined(inputDef.defaultsTo) && _.isUndefined(machine._configuredInputs[inputName])) {
            coercedInputValue = buildLamdaMachine(inputDef.defaultsTo, inputName, machine);
          }
          else {
