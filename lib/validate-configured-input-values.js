...
function getExample (inputDef, inputName, allConfiguredInputValues, machineDef) {
  ...
  if (_.isString(inputDef.like)) {
    ...
    var likeIntersection = rttc.intersection(machineDef.inputs[inputDef.like].example, allConfiguredInputValues[inputDef.like], true, true);
    if (_.isNull(likeIntersection)) {
      ...
      return machineDef.inputs[inputDef.like].example;
    }
  ...
  if (_.isString(inputDef.itemOf)) {
    var otherArrayInputVal = allConfiguredInputValues[inputDef.itemOf];
    ...
    if (!otherArrayInputVal || otherArrayInputVal[0].length < 1) {
    ...
    //  statisfy BOTH schemas-- i.e. the intersection.  This MAY NOT always work
    //  because, at this point, input validation has not run yet.)
    var itemOfIntersection = rttc.intersection(machineDef.inputs[inputDef.itemOf].example[0], otherArrayInputVal[0], true, true);
