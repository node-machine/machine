...
module.exports = function validateConfiguredInputValues(machine){
  ...
  _.each(machine.inputs, function (inputDef, inputName){
    ...
    // var actual = machine._configuredInputs[inputName];
    ...
    //   return displayType;
    // }
    // console.log('\n\nvalue for input %s -- type',inputName,getDisplayType(actual));

    // console.log('validating %s input', inputName);
    ...
    if (_.isUndefined(typeSchema) && inputDef.typeclass){
      ...
      if(inputDef.typeclass === 'dictionary') {
        ...
        if (_.isArray(machine._configuredInputs[inputName])){
          errors.push((function (){
            var err = new Error(util.format('input %s: typeclass: dictionary, but an array was provided: %s', inputName, util.inspect(machine._configuredInputs[inputName], false, null)));
            err.code = 'E_TYPECLASS';
            return err;
          })());
        }
      }
    ...
    // console.log('coercedInputValue:',coercedInputValue, '('+typeof coercedInputValue+')');
